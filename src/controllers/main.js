...
const mainController = {
  ...
  register: (req, res) => {
    res.render('register', { message: session.message });
  },
  ...
  login: (req, res) => {
    const cookieValue = req.cookies.usuarios
    if (cookieValue) {
    ...
    userFound.then(user => {
      if (user) {
        ...
        if (comparePassword) {
          ...
          res.render("login", { email: req.cookies.usuarios, message: req.session.message });
        }
      } else {
        req.session.message = {
          error: `Datos Incorrectos, verifique por favor`
